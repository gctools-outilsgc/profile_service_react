...
module.exports = {
  // You may want 'eval' instead if you prefer to see the compiled
  // output in DevTools.
  ...
  entry: [
    ...
    // one to bring better experience for Create React App users. You can
    // replace the line below with these two lines if you prefer the stock
  ...
  resolve: {
    ...
    alias: {
      ...
      oidcConfig: (process.env.OIDC_ENV === 'local') ?
        path.join(paths.appSrc, 'oidcConfig.local.js') :
        path.join(paths.appSrc, 'oidcConfig.dev.js'),
    },
    plugins: [
      // Prevents users from importing files from outside of src/
      // (or node_modules/).  This often causes confusion because we only
      // process files within src/ with babel.  To fix this, we prevent you
      // from importing files out of src/ -- if you'd like to, please link the
        ...
        oneOf: [
          ...
          // Process JS with Babel.
          {
            ...
            options: {
              ...
              // (not Babel itself).  It enables caching results in
  ...
  plugins: [
    // Automatically creates language strings in .po files in the i18n dir.
    // eslint-disable-next-line max-len
    // https://github.com/gctools-outilsgc/gctools-components/tree/master/packages/i18n-translation-webpack-plugin
